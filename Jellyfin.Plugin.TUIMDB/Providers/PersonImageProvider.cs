...
{
    /// <summary>
    /// Person image provider powered by TUIMDB.
    /// </summary>
    public class PersonImageProvider : IRemoteImageProvider, IHasOrder
    {
        ...
        public PersonImageProvider(ILogger<PersonImageProvider> logger)
        {
            _logger = logger;
            _logger.LogInformation("TUIMDB PersonImageProvider constructed");
        }
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if (!person.TryGetProviderId("TUIMDB", out var personId))
            {
            ...
            {
                _logger.LogError("TUIMDB PersonImageProvider: Plugin configuration is null");
                return Enumerable.Empty<RemoteImageInfo>();
            }
            ...
            _logger.LogDebug(
                "TUIMDB PersonImageProvider image object: {MetadataJson}",
                JsonSerializer.Serialize(imagesObject, _jsonOptions));

            var images = new List<RemoteImageInfo>();

            var orderedImages = imagesObject.Images
                .OrderBy(i => i.Uid == imagesObject.PrimaryImage?.Uid ? 0 : 1)
                .ThenBy(i => i.Order ?? int.MaxValue);

            foreach (var image in orderedImages)
            {
                var imageUrl = $"{config.PeopleImagesUrl}/{image.Name}";
